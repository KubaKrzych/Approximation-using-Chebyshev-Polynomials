...
// help in the form "./aprox -s spl -p test/dane.1 -g myplot -f 5.1 -t 5.7 -n 300"

// to gnuplot plot 'test/dane.1', 'myplot'

// myplot/plot generated by the approximator - points from the approximation
// dane.1 are the arguments and values ​​of the function (x, y)

// spl - our spline-file (table of base functions)
...
main (int argc, char **argv)
{
  ...
  if (inp != NULL) {
    ...
    if (read_pts_failed (inf, &pts)) { // rewriting data from test/dane to arrays in the structure
      fprintf (stderr, "%s: bad contents of points file: %s\n\n", argv[0],inp);
      fclose(inf);  // Points file structure is wrong
      exit (EXIT_FAILURE);
    }
    else
      fclose (inf);
    ...
    make_spl (&pts, &spl); // pts is a structure in points; spl is a structure in splines

    if( spl.n > 0 ) // write to spl: spl->x[i], spl->f[i], spl->f1[i], spl->f2[i], spl->f3[i]
			write_spl (&spl, ouf);
    ...
    fclose(splf); // Close file splf to free memory allocated for this file
  ...
  if (gpt != NULL && n > 1) {
    ...
    for (i = 0; i < n; i++)
      fprintf (gpf, "%g %g\n", fromX + i * dx, value_spl (&spl, fromX + i * dx)); // write from spl to gnufile pair (x, F (x)),
                                                                                  // where F (x) is from the Taylor formula

    fclose (gpf);
    free_spline(&spl);  // free spl struct
  }
  return 0;
}
